...
if [ $SOK_OS == "ERROR" ]; then
    ...
    echo "Contact https://nexusvps..in/order/contact.php to add support for your OS."
    exit 1;
fi

if [ $SOK_OS == "ubuntu2204" ]; then
    /usr/bin/apt -y remove --purge squid squid-.inmon squid-langpack
    rm -rf /etc/squid/
...
echo "Thank you for using nexusvps..in AD proxy installer"
echo "If you want to reinstall AD Proxy Server, check https://nexusvps..in/proxy"
echo 
